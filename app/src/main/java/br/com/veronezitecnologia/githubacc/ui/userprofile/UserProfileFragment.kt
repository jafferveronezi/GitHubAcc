...
import br.com.veronezitecnologia.githubacc.R
...
class UserProfileFragment : Fragment() {
    ...
    private fun setUpView() {
        btPesquisar.setOnClickListener {
            viewModel.pesquisar(edtPesquisar.text.toString())
    ...
    private fun updateUI(user: User?) {
        ...
        if (user != null) {
            Picasso.get().load(user.avatarURL).into(ivImagem)
            tvImagem.text = user.login
        }
    }
}